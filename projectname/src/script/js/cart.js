(function() {
        ...
        $.ajax({
            ...
            $.each(data, function(index, value) {
                ...
                if (id == value.sid) { //遍历判断sid和传入的sid是否相同，方便将那条数据设置到渲染的商品列表中。
                    ...
                    var $clonebox = null;
                    var $clonebox = $('.goods-inner:hidden').clone(true, true);
                    // console.log($('.goods-inner').size())
                    $clonebox.find('.goods-items-pic').find('img').attr('src', value.url);
                    $clonebox.find('.goods-items-pic').find('img').attr('sid', value.sid);
                    $clonebox.find('.goods-items-title').find('a').html(value.goods_title);
                    $clonebox.find('.cost-items').find('strong').html(value.goods_cost);
                    $clonebox.find('.goods-num').find('em').html(count);
                    //计算每个商品的价格。
                    $clonebox.find('.sub-items').find('strong').html((value.goods_cost * count).toFixed(2));
                    $clonebox.css('display', 'block');
                    $('.goods-content-inner tbody').append($clonebox);
                    priceall(); //计算总价的
                }
            });
        })
    ...
    //     $('.sub-items').find('strong').html(singlegoodsprice()); //改变后的价格
    //     // priceall(); //重新计算总和。
    ...
    //     var $dj = parseFloat($('.goods-items-title').find('a').html()); //单价
    //     var $cnum = parseInt($('.details-num').html()); //数量
    ...
    //删除单个商品的函数(委托)
    $('.goods-inner').on('click', '.goods-oper a', function(ev) {

        cookietoarray(); //得到数组,上面的删除cookie需要。
        if (confirm('你确定要删除吗？')) {
            $(this).first().parents('.goods-inner').remove(); //通过当前点击的元素找到整个一行列表，删除
        }
        delgoodslist($(this).first().parents('.goods-inner').find('img').attr('sid'), arrsid);
        // priceall();
    });
